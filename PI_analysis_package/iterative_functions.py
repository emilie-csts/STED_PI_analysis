...
def iterative_fit(f,xdata,ydata,nb_iterations=5,tol=0.05,sigma=50):
    n=0
    minB = []
    maxB = []
    labels_y = ['data']
    ...
    while n<nb_iterations and r_sq<goodness:
        ...
        try:
            for i in range(0,3*n,3):
                ...
                guess[i+1]=((i+1)/(n+1))*(np.max(xdata)-np.min(xdata))+np.min(xdata)
    ...
    if r_sq<goodness:
        ...
        popt = np.zeros(3*n+1)
        popt[i in range(3*n+1)] = np.nan
        pcov = np.zeros(3*n+1)
        pcov[i in range(3*n+1)] = np.nan 
        r_sq = np.nan
        residuals = np.zeros(len(ydata)) 
        residuals[i in range(len(ydata))] = np.nan
